...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default='.',
        help='Parent directory containing the boltz_results_* subfolders'
    )
...
class BoltzResultsProcessor:
    ...
    def __init__(self, config: dict):
        self.config = config
        self.base_dir = Path(config.get('input_dir', '.'))
    ...
    def parse_boltz_results(self) -> pd.DataFrame:
        ...
        for folder in folder_list:
            ...
            predictions_folder = Path(self.base_dir) / folder / 'predictions' / folder_id
            ...
            if not predictions_folder.exists():
                ...
                predictions_folder = Path(self.base_dir) / folder / 'predictions' / folder_id  # Use the full folder name
                if not predictions_folder.exists():
            ...
            # Extract a clean ChEMBL ID from the boltz_results_* folder name (e.g.
            # "boltz_results_CHEMBL121" -> "CHEMBL121") and use it for every row from this folder.
            chembl_match = re.search(r'CHEMBL\d+', folder)
            results_df['chembl_id'] = chembl_match.group(0) if chembl_match else None
    ...
    def collect_pdb_files(self) -> List[str]:
        ...
        for folder in folder_list:
            ...
            predictions_folder1 = Path(self.base_dir) / folder / 'predictions' / folder_id
            predictions_folder2 = Path(self.base_dir) / folder / 'predictions' / folder  # Use full folder name
